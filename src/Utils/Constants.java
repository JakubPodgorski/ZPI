...
public class Constants {
	...
	public static String SELECT_EARNINGS = "SELECT s1.CENAZMARZA - s1.CENA, s1.ILOSCOSOBWPOKOJU From OPCJAOFERTY s1, REZERWACJA s2 WHERE s1.TERMINOD >= ? AND s1.TERMINDO <= ? AND s1.ID=s2.FK_OPCJAOFERTY";
  	public static String SELECT_OFFERS_TIMESTAMP =  "SELECT * FROM OPCJAOFERTY s1, OPISOFERTY s2, REZERWACJA s3 WHERE s1.FK_OPISOFERTY = s2.ID AND s1.TERMINOD >= ? AND s1.TERMINDO <= ? AND s1.ID=s3.FK_OPCJAOFERTY";
